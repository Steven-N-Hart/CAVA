...
def getAnnotation(variant, transcript, reference, prot, mutprot, strict_hgvs=False):
    """
	...
	:param prot:
	:param mutprot:
	:param strict_hgvs:  If False, reverts to old CSN behavior .. if True implements strict HGVS nomenclature
	:return:
	"""
...
def makeProteinString(variant, transcript, reference, prot, mutprot, coord1, strict_hgvs=False):
    if strict_hgvs is True:
        return makeProteinStringHGVS(variant, transcript, reference, prot, mutprot, coord1)
    else:
        return makeProteinStringOriginal(variant, transcript, reference, prot, mutprot, coord1)


# Calculating protein level annotation of the variant
# Original Rahman Code, does not implement current/full HGVS spec . Ter Symbol is X
#
def makeProteinStringOriginal(variant, transcript, reference, prot, mutprot, coord1):
    protcopy = prot[:]
...
# 
def makeProteinStringHGVS(variant, transcript, reference, prot, mutprot, coord1):
    if prot == '': return '', ('.', '.', '.')
    ...
    if prot[0] == 'X':  # Don't have to check for frameshift or SSR if stop codon is first mutated.
        if len(
                mutprot) == 0:  # 3' Extension of unknown length on reference sequence without reference sequence that can lead to extension
    ...
    if (len(variant.alt) - len(variant.ref)) % 3 > 0 or couldbe_delstop_or_ins_extendingstop:
        if len(
                mutprot) == 0:  # Deletion until the end of the protein and whole UTR (since no extra AA picked up from UTR)
    ...
    if len(trim_mutprot) == 0:
        if len(trim_prot) == 1:
            ...
            protpos = str(leftindex) + '-' + str(rightindex)
            return '_p.' + changeTo3lettersTer(trim_prot[0]) + str(leftindex) + '_' + changeTo3lettersTer(
                trim_prot[-1]) + str(rightindex) + 'del', (protpos, trim_prot, '-')
