...
#
# HS: CSN returned  extX instead of extTer, that has to be fixed for HGVSp
# 
	...
	if (len(variant.alt) - len(variant.ref)) % 3 > 0 or couldbe_delstop_or_ins_extendingstop:
		if len(mutprot) == 0:  # Deletion until the end of the protein and whole UTR (since no extra AA picked up from UTR)
			...
			if xindex != -1:  # p.(Arg123LysfsTer34)
				...
				if xindex + 1 == len(prot):  # Stop codon in mutprot at end of original protein
										 # .. either no change to original stop codon, or original prot did not have a stop codon there.
				...
				if xindex == 0 and len(prot) > 1:  # First Modified Base is a stop codon before end (Independent wether mutation is
